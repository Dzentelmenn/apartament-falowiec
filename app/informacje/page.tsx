import PageHero from "@/components/PageHero";

export default function InformacjePage() {
  return (
    <main className="min-h-screen bg-neutral-950 text-white">

      {/* HERO */}
      <PageHero
        title="Informacje"
        subtitle="Wszystkie najważniejsze informacje dotyczące pobytu w apartamencie."
      />

      {/* CONTENT */}
      <section className="mx-auto flex max-w-5xl flex-col gap-6 px-5 py-10">

        {/* LOCATION */}
        <div className="rounded-[32px] bg-white/10 p-8 backdrop-blur-xl">

          <div className="flex items-center gap-4">

            <div className="text-5xl">
              📍
            </div>

            <div>

              <h2 className="text-4xl font-black">
                Lokalizacja
              </h2>

              <p className="mt-2 text-neutral-300">
                Adres apartamentu
              </p>

            </div>

          </div>

          <p className="mt-8 text-2xl font-bold">
            ul. Jagiellońska 10M/33
          </p>

          <p className="mt-3 text-lg text-neutral-300">
            Gdańsk Przymorze
          </p>

          <a
            href="https://maps.app.goo.gl/gFyWzwhzrKZCaBy98"
            target="_blank"
            className="mt-8 inline-block rounded-full bg-white/10 px-6 py-4 text-sm font-medium text-white transition hover:bg-white/20"
          >
            Otwórz mapę
          </a>

        </div>

        {/* WIFI */}
        <div className="rounded-[32px] bg-white/10 p-8 backdrop-blur-xl">

          <div className="flex items-center gap-4">

            <div className="text-4xl">
              📶
            </div>

            <div>

              <h2 className="text-3xl font-black">
                WiFi
              </h2>

              <p className="mt-1 text-neutral-300">
                Dostęp do internetu w mieszkaniu
              </p>

            </div>

          </div>

          <div className="mt-8 space-y-4">

            <div className="rounded-2xl bg-black/30 p-5">

              <p className="text-sm uppercase tracking-widest text-neutral-400">
                Nazwa sieci
              </p>

              <p className="mt-2 text-2xl font-bold">
                NETIASPOT-2.4GHz-Suxz
              </p>

            </div>

            <div className="rounded-2xl bg-black/30 p-5">

              <p className="text-sm uppercase tracking-widest text-neutral-400">
                Hasło
              </p>

              <p className="mt-2 text-2xl font-bold">
                U3c7yxqt
              </p>

            </div>

          </div>

        </div>

        {/* CHECK-IN */}
        <div className="rounded-[32px] bg-white/10 p-8 backdrop-blur-xl">

          <div className="flex items-center gap-4">

            <div className="text-4xl">
              🔑
            </div>

            <div>

              <h2 className="text-3xl font-black">
                Zameldowanie
              </h2>

              <p className="mt-1 text-neutral-300">
                Informacje dotyczące przyjazdu
              </p>

            </div>

          </div>

          <div className="mt-8 rounded-2xl bg-black/30 p-6">

            <p className="text-lg leading-relaxed text-neutral-200">
              Zameldowanie możliwe od godziny
              <span className="font-bold text-white">
                {" "}15:00
              </span>.
            </p>

            <p className="mt-4 text-lg leading-relaxed text-neutral-200">
              Klucze znajdują się w skrytce przy drzwiach wejściowych.
            </p>

          </div>

        </div>

        {/* CHECKOUT */}
        <div className="rounded-[32px] bg-white/10 p-8 backdrop-blur-xl">

          <div className="flex items-center gap-4">

            <div className="text-4xl">
              🚪
            </div>

            <div>

              <h2 className="text-3xl font-black">
                Wymeldowanie
              </h2>

              <p className="mt-1 text-neutral-300">
                Informacje dotyczące wyjazdu
              </p>

            </div>

          </div>

          <div className="mt-8 rounded-2xl bg-black/30 p-6">

            <p className="text-lg leading-relaxed text-neutral-200">
              Prosimy o opuszczenie mieszkania do godziny
              <span className="font-bold text-white">
                {" "}11:00
              </span>.
            </p>

            <p className="mt-4 text-lg leading-relaxed text-neutral-200">
              Klucze należy zostawić z powrotem w skrytce.
            </p>

          </div>

        </div>

        {/* PARKING */}
        <div className="rounded-[32px] bg-white/10 p-8 backdrop-blur-xl">

          <div className="flex items-center gap-4">

            <div className="text-4xl">
              🚗
            </div>

            <div>

              <h2 className="text-3xl font-black">
                Parking
              </h2>

              <p className="mt-1 text-neutral-300">
                Parking dla gości
              </p>

            </div>

          </div>

          <div className="mt-8 rounded-2xl bg-black/30 p-6">

            <p className="text-lg leading-relaxed text-neutral-200">
              Parking znajduje się pod blokiem.
            </p>

            <p className="mt-4 text-lg leading-relaxed text-neutral-200">
              Identyfikator parkingowy należy zostawić
              za przednią szybą pojazdu.
            </p>

          </div>

        </div>

      </section>

    </main>
  );
}